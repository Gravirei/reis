# Plan: 2-3 - Debug Analysis Engine

## Objective
Implement DebugAnalyzer engine that performs 6-step analysis protocol with FR2.1 incomplete implementation root cause analysis.

## Context
The DebugAnalyzer is the core of the debugger. It:
- Executes full 6-step protocol
- Uses IssueClassifier for Step 1
- **FR2.1:** Analyzes WHY features were skipped (executor skip 70%, plan ambiguity 20%, dependency blocker 10%)
- Generates solution options with targeted re-execution for incomplete implementations
- Produces DEBUG_REPORT.md

## Dependencies
- Plan 2-2 (IssueClassifier exists)

## Tasks

<task type="auto">
<name>Implement DebugAnalyzer with FR2.1 incomplete implementation root cause analysis</name>
<files>lib/utils/debug-analyzer.js</files>
<action>
Create comprehensive debug analyzer with FR2.1 support:
...
**What to avoid:**
- Using standard bug analysis for incomplete implementations
- Missing likelihood estimation (70%/20%/10%)
- Not checking git log for missing feature commits
- Skipping plan ambiguity analysis
- Generic solutions for incomplete implementations

**Why:**
- Incomplete implementations need specialized root cause analysis
- Likelihood estimation guides solution selection
- Evidence-based analysis produces accurate recommendations
- Targeted solutions prevent wasted re-implementation
</action>
<verify>
```bash
# Check file exists
test -f lib/utils/debug-analyzer.js && echo "✓ debug-analyzer.js created"

# Verify FR2.1 incomplete implementation analysis
grep -q "analyzeIncompleteImplementation" lib/utils/debug-analyzer.js && echo "✓ FR2.1 analysis method exists"

# Check for likelihood estimation
grep -c "likelihood" lib/utils/debug-analyzer.js
# Should find 15+ occurrences

# Verify three cause analysis methods
grep -q "analyzeExecutorSkip" lib/utils/debug-analyzer.js && \
grep -q "analyzePlanAmbiguity" lib/utils/debug-analyzer.js && \
grep -q "analyzeDependencyBlocker" lib/utils/debug-analyzer.js && \
echo "✓ All three cause analysis methods exist"

# Check for targeted re-execution solution
grep -q "Targeted Re-execution" lib/utils/debug-analyzer.js && echo "✓ Targeted solution included"

# Test syntax
node -c lib/utils/debug-analyzer.js && echo "✓ Syntax valid"
```
</verify>
<done>
- lib/utils/debug-analyzer.js created with full 6-step protocol
- FR2.1 incomplete implementation root cause analysis implemented
- Likelihood estimation: executor skip (70%), plan ambiguity (20%), dependency blocker (10%)
- Three specialized analysis methods for each cause type
- Git log analysis for missing commits
- Plan content analysis for ambiguity
- Dependency and error log checking
- Targeted re-execution solution design
- Time estimation for incomplete implementation fixes
- Evidence-based recommendations
</done>
</task>

## Success Criteria
- ✅ DebugAnalyzer implements full 6-step protocol
- ✅ FR2.1 incomplete implementation analysis complete
- ✅ Likelihood estimation works (70%/20%/10%)
- ✅ Specialized analysis for each cause type
- ✅ Targeted re-execution solution design
...
```bash
# Module exists
test -f lib/utils/debug-analyzer.js && echo "✓ DebugAnalyzer implemented"

# Has FR2.1 support
grep -c "incomplete-implementation" lib/utils/debug-analyzer.js
# Should find 10+ occurrences

# Has likelihood estimation
grep -c "likelihood" lib/utils/debug-analyzer.js
# Should find 15+ occurrences

# Syntax valid
node -c lib/utils/debug-analyzer.js && echo "✓ Valid JavaScript"

# Can be imported
node -e "import('./lib/utils/debug-analyzer.js').then(() => console.log('✓ Module loads'))"
```
