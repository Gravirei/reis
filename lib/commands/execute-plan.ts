...
import { WaveExecutor } from '../utils/wave-executor.js';
import StateManager from '../utils/state-manager.js';
import { loadConfig } from '../utils/config.js';
import { getGitStatus, commitWaveCompletion } from '../utils/git-integration.js';
import { parseDecisionTrees } from '../utils/decision-tree-parser.js';
import { renderDecisionTree } from '../utils/visualizer.js';
import { showKanbanBoard } from '../utils/kanban-renderer.js';
import { sanitizePath, isPathSafe } from '../utils/path-sanitizer.js';
 ...
 */
async function executePlan(args: ExecutePlanArgs): Promise<number> {
  ...
  if (!args.wave) {
    ...
    if (!skipTrees) {
      try {
        const planContent = fs.readFileSync(sanitizedPlanPath, 'utf8');
    ...
    const prompt = `Execute the specific plan at ${sanitizedPlanPath} using the reis_executor subagent. Follow the plan's tasks sequentially, verify each step, and update .planning/STATE.md when complete.`;
    showPrompt(prompt);
    return 0;
  }
  ...
  try {
    ...
    if (!fs.existsSync(sanitizedPlanPath)) {
      showError(`Plan file not found: ${sanitizedPlanPath}`);
    ...
    // Cast needed: runtime state-manager.js still exposes legacy checkpoint APIs
    const stateManager = new StateManager(projectRoot) as any;
    
    // Parse plan
    console.log(`\n📋 Parsing plan: ${sanitizedPlanPath}\n`);
    const planContent = fs.readFileSync(sanitizedPlanPath, 'utf-8');
    ...
    for (let i = 0; i < waves.length; i++) {
      ...
      try {
        ...
        stateManager.createCheckpoint(sanitizedPlanPath, checkpointData);
    ...
    // Final state update
    stateManager.markPlanComplete(sanitizedPlanPath);
 ...
 */
function parsePlanFile(content: string): ParsedWave[] {
  const waves = new Map<number, ParsedWave>();
