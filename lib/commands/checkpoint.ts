...
import * as GitIntegration from '../utils/git-integration.js';
 ...
 * Create a checkpoint
 */
async function createCheckpoint(options: any = {}) {
 ...
 */
async function listCheckpoints(options: any = {}) {
  const projectRoot = process.cwd();
  const planningDir = path.join(projectRoot, '.planning');
 ...
 */
async function checkpoint(options: any = {}) {
