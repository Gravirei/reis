...
import { GateRunner, GateResult, BaseGate } from '../utils/gate-runner.js';
import { GateReporter } from '../utils/gate-reporter.js';
import { SecurityGate, QualityGate, PerformanceGate, AccessibilityGate } from '../utils/gates/index.js';
 ...
 * @returns {GateRunner}
 */
export function createGateRunner(config: any) {
  const runner = new GateRunner({ timeout: config.timeout || 30000 });

  // Register gates based on config
  if (config.security?.enabled !== false) {
    runner.registerGate('security', new SecurityGate(config.security || {}) as any);
  }

  if (config.quality?.enabled !== false) {
    runner.registerGate('quality', new QualityGate(config.quality || {}) as any);
  }

  if (config.performance?.enabled) {
    runner.registerGate('performance', new PerformanceGate(config.performance || {}) as any);
  }

  if (config.accessibility?.enabled) {
    runner.registerGate('accessibility', new AccessibilityGate(config.accessibility || {}) as any);
  }
 ...
 */
export async function runAllGates(options: any = {}) {
  ...
  // Print report
  console.log('');
  reporter.printReport(results as any, summary);
...
/**
 * Run gates for cycle integration
 * @param {Object} options - Gate options
 * @param {boolean} options.verbose - Show detailed output
 * @param {string} options.category - Only run specific category
 * @param {boolean} options.silent - Suppress console output
 * @param {boolean} options.blockOnWarning - Treat warnings as failures
 * @param {number} options.timeout - Override default timeout
 * @returns {Promise<Object>} Gate result
 */
export async function runGatesForCycle(options: any = {}) {
  const projectConfig = loadConfig();
  const gateConfig = mergeConfig(projectConfig?.gates || {});
  ...
  if (!gateConfig.enabled) {
    return {
      passed: true,
      skipped: true,
      reason: 'Gates disabled in configuration',
      results: [],
      summary: { total: 0, passed: 0, warning: 0, failed: 0, skipped: 0 }
    };
  }

  const runner = createGateRunner({
    ...gateConfig,
    timeout: options.timeout || gateConfig.timeout
  });

  // Add event listeners for progress (only if not silent and verbose)
  ...
  let passed = runner.hasPassed();
  if (options.blockOnWarning && runner.hasWarnings()) {
    passed = false;
  }
  ...
  return {
    ...
    summary,
    config: {
      blockOnFail: gateConfig.blockOnFail,
      blockOnWarning: options.blockOnWarning || (gateConfig as any).blockOnWarning
    }
  };
}
 ...
 * @param {Object} options - Command options
 */
export async function runCategoryGates(category: string, options: any = {}) {
 ...
 * @param {Object} options - Command options
 */
function showGateStatus(options: any = {}) {
  const projectConfig = loadConfig();
  const gateConfig = mergeConfig(projectConfig?.gates || {});
  ...
  for (const category of categories) {
    const catConfig: any = gateConfig[category] || {};
    ...
    if (enabled && typeof catConfig === 'object') {
      for (const [check, checkConfig] of Object.entries(catConfig)) {
        ...
        if (typeof checkConfig === 'object') {
          const checkEnabled = (checkConfig as any).enabled !== false;
 ...
 * @param {Object} options - Command options
 */
export async function gateCommand(subcommand: string | undefined, options: any = {}) {
  try {
    switch (subcommand) {
      case 'check':
      case 'run':
