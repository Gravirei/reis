import { showPrompt, showError, showInfo, checkPlanningDir, validatePhaseNumber } from '../utils/command-helpers.js';
...
function plan(args: PlanArgs, options: PlanOptions = {}): number {
  ...
  const depth = args.depth ? parseInt(args.depth as string, 10) : Infinity;
  ...
  if (showTrees || treesJson) {
    ...
    if (trees.length === 0) {
      ...
      if (treesJson) {
        const { toJSON } = require('../utils/decision-tree-parser.js');
        // Output JSON format
        const jsonOutput = trees.map((tree: any) => toJSON(tree));
 ...
 * @param phasePath - Path to phases directory
 * @param phase - Phase number
 * @returns Array of parsed trees
 */
function findAndParseTrees(phasePath: string, phase: number): any[] {
  const trees: any[] = [];
