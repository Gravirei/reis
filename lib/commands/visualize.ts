...
const visualizer = {
  createProgressBar(current: number, total: number, options: any = {}) {
...
function colorizeStatus(status: string) {
...
async function render(type, options) {
  try {
    ...
    const state = await (stateManager as any).getState();
 ...
 */
async function displayDependencies(options: any = {}) {
 ...
 * @param {Object} options - Display options
 */
async function displayTimeline(options: any = {}) {
  if (!options.compact) {
    console.log(chalk.bold('\n╔════════════════════════════════════════════════════════════╗'));
