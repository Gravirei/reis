...
import { runAllGates } from './gate.js';
 ...
 */
async function verify(target: string, options: VerifyOptions = {}): Promise<void> {
  ...
  try {
    ...
    // Step 7: Optionally run quality gates after verification
    if (result.passed && options.withGates && !options.skipGates) {
 ...
 */
function generateVerificationPrompt(planPath: string, planData: ParsedPlan, options: VerifyOptions): string {
 ...
 */
async function invokeVerifier(prompt: string, planPath: string, planData: ParsedPlan, options: VerifyOptions): Promise<VerificationResult> {
