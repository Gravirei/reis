...
async function displayProgress(state, metrics, options) {
  ...
  if (metrics && Object.keys(metrics).length > 0) {
    ...
    if (metrics.wavesCompleted !== undefined) {
      console.log(`  Waves Completed: ${metrics.wavesCompleted}/${metrics.totalWaves || '?'}`);
    }
...
async function displayMetrics(state, metrics, options) {
  ...
  if (metrics.wavesCompleted !== undefined) console.log(`  Waves Completed: ${metrics.wavesCompleted}`);
  if (metrics.tasksCompleted !== undefined) console.log(`  Tasks Completed: ${metrics.tasksCompleted}`);
}
...
async function render(type, options) {
  try {
    ...
    const metricsTracker = new MetricsTracker(config);
    const state = await stateManager.getState();
    let metrics = {};
    try {
      const history = await metricsTracker.getExecutionHistory();
      if (history && history.length > 0) {
        const completed = history.filter(h => h.status === 'success');
        metrics.wavesCompleted = completed.length;
        metrics.totalWaves = history.length;
        metrics.successRate = completed.length / history.length;
        let totalTasks = 0;
        history.forEach(h => { if (h.tasksCompleted) totalTasks += h.tasksCompleted; });
