...
import { getGitStatus, getCommitDiff } from '../utils/git-integration.js';
 ...
 */
async function resume(args: ResumeArgs, options: ResumeOptions = {}): Promise<number> {
 ...
 */
async function smartResume(stateManager: StateManager, config: ReisConfig, args: ResumeArgs): Promise<number> {
  const state = stateManager.state;

  // Backward compatibility - if no state data, use old prompt-based behavior
  if (!state.activeWave && state.checkpoints.length === 0 && state.recentActivity.length === 0 && state.blockers.length === 0 && state.nextSteps.length === 0) {
 ...
 */
function listResumePoints(stateManager: StateManager, config: ReisConfig): number {
  const state = stateManager.state;
 ...
 * Resume from specific checkpoint
 */
async function resumeFromCheckpoint(checkpointName: string, stateManager: StateManager, config: ReisConfig, args: ResumeArgs): Promise<number> {
  const state = stateManager.state;
 ...
 * Continue incomplete wave
 */
async function continueWave(stateManager: StateManager, config: ReisConfig, args: ResumeArgs): Promise<number> {
  const state = stateManager.state;
