...
import { showError, showSuccess, showInfo, checkPlanningDir } from '../utils/command-helpers.js';
 ...
 */
async function debugCommand(target: string | undefined, options: any = {}) {
 ...
 */
function detectIssueType(inputContent: string) {
 ...
 */
function buildDebuggerPrompt(debugInput: string, context: any, issueType: string, options: any) {
 ...
 */
async function invokeDebugger(prompt: string, options: any): Promise<any> {
 ...
 */
function displayReportSummary(reportPath: string) {
  if (!fs.existsSync(reportPath)) {
 ...
 */
function displayFixPlanSummary(fixPlanPath: string, issueType: string) {
  if (!fs.existsSync(fixPlanPath)) {
