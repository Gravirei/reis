...
function saveState(cycleData) {
  try {
    ...
    const state = {
      ...
      // Preserve gate-related state
      gateResult: cycleData.gateResult || null,
      executionResult: cycleData.executionResult || null,
      verificationResult: cycleData.verificationResult || null
    };
...
function isResumable() {
  ...
  // RESEARCHING added for --research and --full-research flags
  const resumableStates = ['RESEARCHING', 'PLANNING', 'REVIEWING', 'EXECUTING', 'VERIFYING', 'GATING', 'DEBUGGING', 'FIXING'];
