...
class DependencyParser {
  ...
  parseContent(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Check for dependency comment
      const depMatch = this._matchDependency(line);
      if (depMatch !== null) {
        if (currentWave) {
          // Apply to current wave (inline/after wave header)
          currentWave.dependencies = [...new Set([...currentWave.dependencies, ...depMatch])];
          dependencies.set(currentWave.id, currentWave.dependencies);
        } else {
          // Store as pending for next wave (before wave header)
          pendingDependencies = depMatch;
        }
        continue;
      }

      // Check for parallel group comment
      const groupMatch = this._matchGroup(line);
      if (groupMatch) {
        if (currentWave) {
          // Apply to current wave (inline/after wave header)
          currentWave.parallelGroup = groupMatch;
          groups.set(currentWave.id, groupMatch);
        } else {
          // Store as pending for next wave (before wave header)
