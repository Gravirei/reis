...
class StateManager {
  ...
  parseState(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (currentSection === 'wave' && line.startsWith('**')) {
        state.activeWave = this.parseWaveBlock(lines.slice(i, i + 10));
      } else if (currentSection === 'activity' && line.startsWith('- ')) {
        state.recentActivity.push(line.substring(2).trim());
      } else if (currentSection === 'checkpoints' && line.startsWith('- **')) {
        ...
        if (cpMatch) {
          ...
          // Look ahead for commit and wave info
          for (let j = i + 1; j < Math.min(i + 5, lines.length); j++) {
            const nextLine = lines[j].trim();
            if (nextLine.startsWith('- Commit:')) {
              const commitMatch = nextLine.match(/Commit:\s*`(.+?)`/);
              if (commitMatch) checkpoint.commit = commitMatch[1];
            } else if (nextLine.startsWith('- Wave:')) {
              const waveMatch = nextLine.match(/Wave:\s*(.+)/);
              if (waveMatch) checkpoint.wave = waveMatch[1];
            } else if (nextLine.startsWith('- **')) {
              break; // Next checkpoint, stop looking
            }
          }
          
          state.checkpoints.push(checkpoint);
        }
      } else if (currentSection === 'nextSteps' && line.match(/^\d+\.\s+/)) {
        // Parse numbered next steps
        state.nextSteps.push(line.replace(/^\d+\.\s+/, '').trim());
      } else if (currentSection === 'blockers' && line.startsWith('- ') && !line.startsWith('- _')) {
        // Parse blockers (skip "_None_" lines)
        state.blockers.push(line.substring(2).trim());
      } else if (currentSection === 'notes' && line.startsWith('- ') && !line.startsWith('- _')) {
        // Parse notes (skip "_None_" lines)
        state.notes.push(line.substring(2).trim());
