...
function renderDecisionTree(treeData, options = {}) {
  ...
  if (!treeData || !treeData.root) {
    return chalk.red('Invalid tree data');
  }
  ...
  // Tree header
  lines.push('');
  lines.push(chalk.bold.cyan(`Decision Tree: ${treeData.name}`));
  lines.push(chalk.bold(treeData.root));
  lines.push('');
  ...
  const renderBranch = (branch, level, isLast, prefix) => {
    ...
    if (showMetadata && branch.metadata) {
      ...
      if (branch.metadata.weight) {
        badges.push(chalk.bgYellow.black(` WEIGHT: ${branch.metadata.weight} `));
      }
      if (branch.metadata.priority) {
        ...
        badges.push(colorFn(` ${branch.metadata.priority.toUpperCase()} `));
      }
      if (branch.metadata.risk) {
        ...
        badges.push(colorFn(` RISK: ${branch.metadata.risk.toUpperCase()} `));
      }
    ...
    if (branch.condition && branch.condition !== 'ELSE') {
      text = chalk.dim.italic(`[IF: ${branch.condition}] `) + text;
    } else if (branch.condition === 'ELSE') {
      text = chalk.dim.italic('[ELSE] ') + text;
    }

    // Add the line
    lines.push(prefix + chalk.gray(connector) + ' ' + text);
    ...
    if (branch.children && branch.children.length > 0) {
      ...
      if (shouldExpand && level < depth) {
        ...
        lines.push(prefix + chalk.gray(continueLine) + ' ' + chalk.dim('[...]'));
      }
    }
  };
