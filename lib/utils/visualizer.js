...
function renderDecisionTree(treeData, options = {}) {
  ...
  const renderBranch = (branch, level, isLast, prefix) => {
    ...
    if (branch.children && branch.children.length > 0) {
      ...
      } else if (branch.children.length > 0 && level >= depth) {
        // Show collapse indicator when depth limit reached
        const hiddenCount = countNodes({ branches: branch.children });
        const indicator = '[...]';
        const continueLineText = noColor ? continueLine : chalk.gray(continueLine);
        lines.push(prefix + continueLineText + ' ' + (noColor ? indicator : chalk.dim(indicator)));
      } else if (branch.children.length > 0) {
        // Show collapse indicator with count of hidden children
        const hiddenCount = countNodes({ branches: branch.children });
        const indicator = hiddenCount === 1 
          ? (noColor ? '[+] 1 hidden node' : chalk.dim('[+] 1 hidden node'))
          : (noColor ? `[+] ${hiddenCount} hidden nodes` : chalk.dim(`[+] ${hiddenCount} hidden nodes`));
        const continueLineText = noColor ? continueLine : chalk.gray(continueLine);
        lines.push(prefix + continueLineText + ' ' + indicator);
      }
    }
  };
