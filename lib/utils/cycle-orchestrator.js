const fs = require('fs');
const path = require('path');
const chalk = require('chalk');
const stateManager = require('./cycle-state-manager');
const { parseDecisionTrees } = require('./decision-tree-parser');
const { renderDecisionTree } = require('./visualizer');
const { checkPlanningDir } = require('./command-helpers');
const { invokeSubagent } = require('./subagent-invoker');
const { PlanReviewer } = require('./plan-reviewer');

/**
 * Cycle Orchestrator
 * Orchestrates the complete PLAN → EXECUTE → VERIFY → DEBUG → FIX cycle
 */

/**
 * Run complete cycle for a phase or plan
 * @param {number|string} phaseOrPlan - Phase number or path to PLAN.md
 * @param {Object} options - Cycle options
 * @param {number} options.maxAttempts - Maximum debug/fix attempts (default: 3)
 * @param {boolean} options.autoFix - Auto-apply fixes without confirmation
 * @param {boolean} options.continueOnFail - Don't stop on verification failure
 * @param {boolean} options.verbose - Show detailed output
 * @param {boolean} options.resume - Resume from interrupted cycle
 * @param {boolean} options.research - Run reis_scout before planning
 * @param {boolean} options.fullResearch - Run reis_analyst + reis_scout before planning
 * @param {boolean} options.quick - Fast mode: skip research, review, and gates
 * @returns {Object} Cycle result
 */
async function runCycle(phaseOrPlan, options = {}) {
  const startTime = Date.now();
  
  // Set defaults
  options.maxAttempts = options.maxAttempts || 3;
  options.autoFix = options.autoFix || false;
  options.continueOnFail = options.continueOnFail || false;
  options.verbose = options.verbose || false;
  
  // Handle quick mode - skip research, review, and gates
  if (options.quick) {
    console.log(chalk.yellow('\n⚡ Quick mode enabled - skipping research, review, and gates\n'));
    options.skipReview = true;
    options.skipGates = true;
    options.research = false;
    options.fullResearch = false;
  }
  
  try {
    // Validate REIS project
    if (!checkPlanningDir()) {
      throw new Error('Not a REIS project. Run "reis new" or "reis map" first.');
    }
    
    // Handle resume
    if (options.resume) {
      return await resumeCycle(options);
    }
    
    // Initialize new cycle
    const planPath = await initializeCycle(phaseOrPlan, options);
    
    // Step 0: Research (optional, before planning)
    if (options.fullResearch) {
      await executeFullResearchStep(phaseOrPlan, options);
    } else if (options.research) {
      await executeResearchStep(phaseOrPlan, options);
    }
    
    // Step 1: Planning
    await executePlanningStep(planPath, options);
    
    // Step 2: Review (validates plans against codebase)
    if (!options.skipReview) {
      const reviewResult = await executeReviewStep(planPath, options);
      if (!reviewResult.success) {
        // Handle review failure
        console.log(chalk.red('\n❌ Plan review failed. Fix issues before continuing.'));
        return reviewResult;
      }
    }
    
    // Step 3: Executing
    await executeExecutionStep(planPath, options);
    
    // Step 3: Verifying (with debug/fix loop)
    const verifyResult = await executeVerificationLoop(planPath, options);
    
    // Step 4: Complete
    await executeCompletionStep(planPath, verifyResult, options);
    
    const duration = Math.floor((Date.now() - startTime) / 1000);
    
    return {
      success: true,
      phase: stateManager.loadState()?.phase,
      planPath,
      duration,
      attempts: stateManager.loadState()?.attempts || 0
    };
    
  } catch (error) {
    // Save error state
    stateManager.setLastError(error);
    stateManager.updateState('FAILED', 'failure', error.message);
    
    throw error;
  }
}

/**
 * Resume interrupted cycle
 */
async function resumeCycle(options) {
  const state = stateManager.loadState();
  
  if (!state || !stateManager.isResumable()) {
    throw new Error('No resumable cycle found');
  }
  
  console.log(chalk.blue(`\n🔄 Resuming cycle from ${state.currentState}`));
  console.log(chalk.gray(`Phase: ${state.phase}`));
  console.log(chalk.gray(`Attempts: ${state.attempts}/${state.maxAttempts}\n`));
  
  // Resume from current state
  switch (state.currentState) {
    case 'RESEARCHING':
      // Resume research if flags are set, otherwise skip to planning
      if (options.fullResearch) {
        await executeFullResearchStep(state.phase, options);
      } else if (options.research) {
        await executeResearchStep(state.phase, options);
      }
      await executePlanningStep(state.planPath, options);
      if (!options.skipReview) {
        const reviewResult = await executeReviewStep(state.planPath, options);
        if (!reviewResult.success) {
          return reviewResult;
        }
      }
      await executeExecutionStep(state.planPath, options);
      await executeVerificationLoop(state.planPath, options);
      break;
    
    case 'PLANNING':
      await executePlanningStep(state.planPath, options);
      if (!options.skipReview) {
        const reviewResult = await executeReviewStep(state.planPath, options);
        if (!reviewResult.success) {
          return reviewResult;
        }
      }
      await executeExecutionStep(state.planPath, options);
      await executeVerificationLoop(state.planPath, options);
      break;
      
    case 'REVIEWING':
      if (!options.skipReview) {
        const reviewResult = await executeReviewStep(state.planPath, options);
        if (!reviewResult.success) {
          return reviewResult;
        }
      }
      await executeExecutionStep(state.planPath, options);
      await executeVerificationLoop(state.planPath, options);
      break;
      
    case 'EXECUTING':
      await executeExecutionStep(state.planPath, options);
      await executeVerificationLoop(state.planPath, options);
      break;
      
    case 'VERIFYING':
    case 'DEBUGGING':
    case 'FIXING':
      await executeVerificationLoop(state.planPath, options);
      break;
      
    default:
      throw new Error(`Cannot resume from state: ${state.currentState}`);
  }
  
  await executeCompletionStep(state.planPath, { success: true }, options);
  
  return {
    success: true,
    phase: state.phase,
    planPath: state.planPath,
    resumed: true
  };
}

/**
 * Initialize cycle state
 */
async function initializeCycle(phaseOrPlan, options) {
  // Determine plan path
  let planPath;
  
  if (typeof phaseOrPlan === 'number' || /^\d+$/.test(phaseOrPlan)) {
    // Phase number provided
    const phase = parseInt(phaseOrPlan);
    planPath = path.join(process.cwd(), '.planning', `phase-${phase}.PLAN.md`);
  } else if (phaseOrPlan) {
    // Plan path provided
    planPath = phaseOrPlan;
  } else {
    throw new Error('Phase number or plan path is required');
  }
  
  // Check if plan exists
  if (!fs.existsSync(planPath)) {
    // Offer to generate plan
    const error = new Error(`Plan not found: ${planPath}`);
    error.code = 'PLAN_NOT_FOUND';
    error.suggestion = `Run "reis plan ${phaseOrPlan}" to generate it`;
    throw error;
  }
  
  // Initialize state
  const cycleState = {
    phase: phaseOrPlan,
    planPath,
    currentState: 'PLANNING',
    startTime: new Date().toISOString(),
    attempts: 0,
    maxAttempts: options.maxAttempts,
    options: {
      autoFix: options.autoFix,
      verbose: options.verbose,
      continueOnFail: options.continueOnFail
    },
    history: [],
    lastError: null,
    completeness: 0
  };
  
  stateManager.saveState(cycleState);
  
  return planPath;
}

/**
 * Step 0a: Research (optional)
 * Runs reis_scout for phase-specific research
 */
async function executeResearchStep(phaseOrPlan, options) {
  stateManager.updateState('RESEARCHING', 'pending', 'Running phase research');
  
  if (options.verbose) {
    console.log(chalk.cyan('\n🔍 Running Phase Research...'));
  }
  
  try {
    // Ensure research directory exists
    const researchDir = path.join(process.cwd(), '.planning', 'research');
    if (!fs.existsSync(researchDir)) {
      fs.mkdirSync(researchDir, { recursive: true });
    }
    
    const phase = typeof phaseOrPlan === 'number' || /^\d+$/.test(phaseOrPlan) 
      ? parseInt(phaseOrPlan) 
      : 'custom';
    
    const result = await invokeSubagent('reis_scout', {
      phase: phaseOrPlan,
      verbose: options.verbose,
      timeout: options.timeout || 300000,
      additionalContext: {
        outputPath: path.join(researchDir, `phase-${phase}-research.md`)
      }
    });
    
    if (!result.success) {
      // Research failure is non-blocking by default
      console.log(chalk.yellow('  ⚠️ Research completed with issues'));
      stateManager.updateState('RESEARCHING', 'warning', 'Research completed with issues');
    } else {
      console.log(chalk.green('  ✓ Research complete'));
      stateManager.updateState('RESEARCHING', 'success', 'Research complete');
    }
    
    return result;
  } catch (error) {
    stateManager.updateState('RESEARCHING', 'warning', error.message);
    // Don't throw - research is optional, allow cycle to continue
    console.log(chalk.yellow(`  ⚠️ Research failed: ${error.message}`));
    return { success: false, error };
  }
}

/**
 * Step 0b: Full Research (optional)
 * Runs reis_analyst (project-level) + reis_scout (phase-level)
 */
async function executeFullResearchStep(phaseOrPlan, options) {
  stateManager.updateState('RESEARCHING', 'pending', 'Running full research');
  
  if (options.verbose) {
    console.log(chalk.cyan('\n🔬 Running Full Research Suite...'));
  }
  
  try {
    // Step 1: Project-level research (if context.md doesn't exist)
    const contextPath = path.join(process.cwd(), '.planning', 'context.md');
    if (!fs.existsSync(contextPath)) {
      console.log(chalk.gray('  Running project analysis (reis_analyst)...'));
      try {
        await invokeSubagent('reis_analyst', {
          verbose: options.verbose,
          timeout: options.timeout || 300000,
          additionalContext: { outputPath: contextPath }
        });
        console.log(chalk.green('  ✓ Project analysis complete'));
      } catch (analystError) {
        console.log(chalk.yellow(`  ⚠️ Project analysis failed: ${analystError.message}`));
        // Continue with phase research even if analyst fails
      }
    } else {
      console.log(chalk.gray('  Project context exists, skipping reis_analyst'));
    }
    
    // Step 2: Phase-level research
    console.log(chalk.gray('  Running phase research (reis_scout)...'));
    await executeResearchStep(phaseOrPlan, options);
    
    stateManager.updateState('RESEARCHING', 'success', 'Full research complete');
    return { success: true };
  } catch (error) {
    stateManager.updateState('RESEARCHING', 'warning', error.message);
    console.log(chalk.yellow(`  ⚠️ Full research failed: ${error.message}`));
    return { success: false, error };
  }
}

/**
 * Step 1: Planning
 */
async function executePlanningStep(planPath, options) {
  stateManager.updateState('PLANNING', 'pending', 'Validating plan');
  
  if (options.verbose) {
    console.log(chalk.gray(`\n📋 Planning Step`));
    console.log(chalk.gray(`  Plan: ${planPath}`));
  }
  
  try {
    // Validate plan exists and has content
    if (!fs.existsSync(planPath)) {
      const error = new Error(`Plan not found: ${planPath}`);
      error.code = 'PLAN_NOT_FOUND';
      throw error;
    }
    
    const planContent = fs.readFileSync(planPath, 'utf8');
    if (planContent.length < 100) {
      const error = new Error('Plan appears to be empty or invalid');
      error.code = 'INVALID_PLAN';
      throw error;
    }
    
    // Validate plan structure (basic check)
    if (!planContent.includes('## Task') && !planContent.includes('<task')) {
      const error = new Error('Plan does not contain any tasks');
      error.code = 'INVALID_PLAN';
      error.suggestion = 'Ensure plan follows REIS PLAN.md format';
      throw error;
    }
    
    stateManager.updateState('PLANNING', 'success', 'Plan validated');
    
    if (options.verbose) {
      console.log(chalk.green('  ✓ Plan validated\n'));
    }
  } catch (error) {
    stateManager.updateState('PLANNING', 'failure', error.message);
    throw error;
  }
}

/**
 * Execute review step - validates plans against codebase
 * @param {string} planPath - Path to plan file or directory
 * @param {Object} options - Review options
 * @returns {Object} Review result
 */
async function executeReviewStep(planPath, options = {}) {
  stateManager.updateState('REVIEWING', 'pending', 'Reviewing plans against codebase');
  
  if (options.verbose) {
    console.log(chalk.cyan('\n📋 Reviewing Plans...'));
  }
  
  if (options.skipReview) {
    stateManager.updateState('REVIEWING', 'skipped', 'Review skipped by user');
    return { success: true, skipped: true };
  }
  
  try {
    const reviewer = new PlanReviewer(process.cwd(), {
      autoFix: options.autoFix,
      strict: options.strict
    });
    
    let result;
    if (planPath.endsWith('.md')) {
      result = await reviewer.reviewPlan(planPath);
    } else {
      result = await reviewer.reviewAllPlans(planPath);
    }
    
    stateManager.setReviewResult(result);
    
    const hasIssues = result.issues && result.issues.length > 0;
    const hasCritical = result.issues?.some(i => i.severity === 'critical');
    
    if (hasCritical) {
      stateManager.updateState('REVIEWING', 'failure', 'Critical issues found');
      return { success: false, ...result };
    } else if (hasIssues) {
      stateManager.updateState('REVIEWING', 'warning', `${result.issues.length} issues found`);
      return { success: true, hasWarnings: true, ...result };
    } else {
      stateManager.updateState('REVIEWING', 'success', 'All plans validated');
      return { success: true, ...result };
    }
  } catch (error) {
    stateManager.updateState('REVIEWING', 'failure', error.message);
    return { success: false, error: error.message };
  }
}

/**
 * Step 2: Executing
 */
async function executeExecutionStep(planPath, options) {
  stateManager.updateState('EXECUTING', 'pending', 'Running plan');
  
  // Show decision trees before execution
  await showDecisionTreesAtTransition(planPath, 'PLAN → EXECUTE', options);
  
  if (options.verbose) {
    console.log(chalk.gray(`\n⚙️  Execution Step`));
  }
  
  try {
    // For now, we'll use require to call the execute-plan command
    // In a real implementation, this would spawn the executor subagent
    const executePlan = require('../commands/execute-plan');
    
    // Execute the plan using reis_executor subagent
    if (options.verbose) {
      console.log(chalk.gray('  Executing plan tasks...'));
    }
    
    // Invoke reis_executor with the plan
    const result = await invokeSubagent('reis_executor', {
      planPath: planPath,
      verbose: options.verbose,
      timeout: options.timeout || 600000,
      additionalContext: {
        autoCommit: options.autoCommit !== false,
        phase: stateManager.loadState()?.phase
      }
    });

    if (!result.success) {
      throw new Error(result.error?.message || 'Execution failed');
    }

    stateManager.setExecutionResult(result);
    
    stateManager.updateState('EXECUTING', 'success', 'Plan executed');
    
    if (options.verbose) {
      console.log(chalk.green('  ✓ Plan executed\n'));
    }
    
  } catch (error) {
    stateManager.updateState('EXECUTING', 'failure', error.message);
    const executionError = new Error(`Execution failed: ${error.message}`);
    executionError.code = 'EXECUTION_FAILED';
    executionError.originalError = error;
    throw executionError;
  }
}

/**
 * Step 3: Verification Loop (with debug/fix)
 */
async function executeVerificationLoop(planPath, options) {
  let attempt = stateManager.loadState()?.attempts || 0;
  
  while (attempt < options.maxAttempts) {
    // Run verification
    const verifyResult = await executeVerificationStep(planPath, options);
    
    // Check if verification passed
    if (verifyResult.success && verifyResult.completeness >= 100) {
      // Run gate phase after successful verification
      const gateResult = await executeGateStep(planPath, options);
      
      if (!gateResult.success && !gateResult.skipped) {
        // Gate failure - check if we should block
        const { loadConfig } = require('./config');
        const config = loadConfig();
        const blockOnFail = config?.gates?.blockOnFail !== false;
        
        if (blockOnFail) {
          // Format gate issues for debugging
          const gateIssues = formatGateIssues(gateResult);
          
          // Return as failure to trigger debug cycle
          return {
            success: false,
            completeness: 100, // Verification passed
            issues: gateIssues,
            gateFailure: true,
            gateResults: gateResult
          };
        } else {
          // Gates failed but blockOnFail=false
          if (options.verbose) {
            console.log(chalk.yellow('\n⚠️  Gates failed but blockOnFail=false, continuing...'));
          }
        }
      }
      
      // Continue with success return
      return verifyResult;
    }
    
    // Check if we should continue on failure
    if (options.continueOnFail) {
      console.log(chalk.yellow('\n⚠️  Verification failed but continuing (--continue-on-fail)\n'));
      return verifyResult;
    }
    
    // Verification failed - enter debug/fix loop
    console.log(chalk.yellow(`\n⚠️  Verification failed (${verifyResult.completeness}% complete)`));
    console.log(chalk.yellow(`   Attempt ${attempt + 1}/${options.maxAttempts}\n`));
    
    // Increment attempt counter
    attempt++;
    stateManager.incrementAttempts();
    
    // Check if max attempts reached
    if (attempt >= options.maxAttempts) {
      console.log(chalk.red(`❌ Max attempts reached (${options.maxAttempts})`));
      console.log(chalk.yellow('\nOptions:'));
      console.log(chalk.gray('  1. Review verification output'));
      console.log(chalk.gray('  2. Fix issues manually'));
      console.log(chalk.gray('  3. Increase max attempts: reis cycle --max-attempts 5'));
      console.log(chalk.gray('  4. Skip verification: reis cycle --continue-on-fail\n'));
      
      const error = new Error('Max verification attempts reached');
      error.code = 'MAX_ATTEMPTS_REACHED';
      error.attempts = attempt;
      error.maxAttempts = options.maxAttempts;
      throw error;
    }
    
    // Run debugging step
    const debugResult = await executeDebuggingStep(planPath, verifyResult, options);
    
    // Run fixing step
    await executeFixingStep(planPath, debugResult, options);
    
    // Loop back to verification
    console.log(chalk.blue('\n🔄 Re-running verification...\n'));
  }
  
  throw new Error('Verification loop exited unexpectedly');
}

/**
 * Step 3a: Verification
 */
async function executeVerificationStep(planPath, options) {
  stateManager.updateState('VERIFYING', 'pending', 'Running verification');
  
  // Show decision trees before verification
  await showDecisionTreesAtTransition(planPath, 'EXECUTE → VERIFY', options);
  
  if (options.verbose) {
    console.log(chalk.gray(`\n✓ Verification Step`));
  }
  
  try {
    // Run verification using reis_verifier subagent
    const lastPlanPath = planPath;
    
    // Read plan data for success criteria
    let planData = null;
    try {
      const planContent = fs.readFileSync(planPath, 'utf8');
      // Extract success criteria from plan (basic parsing)
      const criteriaMatch = planContent.match(/## Success Criteria\n([\s\S]*?)(?=\n##|$)/);
      if (criteriaMatch) {
        planData = { successCriteria: criteriaMatch[1].trim().split('\n').filter(l => l.trim()) };
      }
    } catch (e) {
      // Ignore parsing errors
    }

    const verifyResult = await invokeSubagent('reis_verifier', {
      planPath: lastPlanPath,
      verbose: options.verbose,
      timeout: options.timeout || 300000,
      additionalContext: {
        executionResult: stateManager.getExecutionResult(),
        successCriteria: planData?.successCriteria || []
      }
    });

    const completeness = verifyResult.metadata?.completeness || (verifyResult.success ? 100 : 0);
    const issues = verifyResult.metadata?.issues || [];
    const missing = verifyResult.metadata?.missing || [];
    
    stateManager.updateCompleteness(completeness);
    
    const result = {
      success: completeness >= 100,
      completeness,
      issues,
      missing
    };
    
    // Store verification result for debugging step
    stateManager.setVerificationResult(result);
    
    if (result.success) {
      stateManager.updateState('VERIFYING', 'success', `${completeness}% complete`);
      if (options.verbose) {
        console.log(chalk.green(`  ✓ Verification passed (${completeness}%)\n`));
      }
    } else {
      stateManager.updateState('VERIFYING', 'failure', `${completeness}% complete`);
      if (options.verbose) {
        console.log(chalk.yellow(`  ⚠️  Verification failed (${completeness}%)\n`));
      }
    }
    
    return result;
    
  } catch (error) {
    stateManager.updateState('VERIFYING', 'failure', error.message);
    const verifyError = new Error(`Verification failed: ${error.message}`);
    verifyError.code = 'VERIFICATION_FAILED';
    verifyError.originalError = error;
    throw verifyError;
  }
}

/**
 * Step 3b: Debugging
 */
async function executeDebuggingStep(planPath, verifyResult, options) {
  stateManager.updateState('DEBUGGING', 'pending', 'Analyzing failures');
  
  // Show decision trees before debugging
  await showDecisionTreesAtTransition(planPath, 'VERIFY → DEBUG', options);
  
  if (options.verbose) {
    console.log(chalk.gray(`\n🔍 Debugging Step`));
  }
  
  try {
    // Get verification issues
    const issues = verifyResult.issues || [];
    const lastPlanPath = planPath;
    
    // Run debug analysis using reis_debugger subagent
    const debugResult = await invokeSubagent('reis_debugger', {
      verbose: options.verbose,
      timeout: options.timeout || 300000,
      additionalContext: {
        verificationResult: stateManager.getVerificationResult(),
        issues: issues,
        planPath: lastPlanPath
      }
    });

    if (!debugResult.success) {
      throw new Error('Debug analysis failed: ' + (debugResult.error?.message || 'Unknown error'));
    }
    
    // Add artifact paths to result
    debugResult.artifacts = debugResult.artifacts || {};
    debugResult.artifacts.reportPath = debugResult.artifacts?.reportPath || '.planning/debug/DEBUG_REPORT.md';
    debugResult.artifacts.fixPlanPath = debugResult.artifacts?.fixPlanPath || '.planning/debug/FIX_PLAN.md';
    
    // For backward compatibility, also set at top level
    debugResult.reportPath = debugResult.artifacts.reportPath;
    debugResult.fixPlanPath = debugResult.artifacts.fixPlanPath;
    
    stateManager.updateState('DEBUGGING', 'success', 'Fix plan generated');
    
    if (options.verbose) {
      console.log(chalk.green('  ✓ Debug analysis complete\n'));
    }
    
    return debugResult;
    
  } catch (error) {
    stateManager.updateState('DEBUGGING', 'failure', error.message);
    const debugError = new Error(`Debugging failed: ${error.message}`);
    debugError.code = 'DEBUG_FAILED';
    debugError.originalError = error;
    throw debugError;
  }
}

/**
 * Step 3c: Fixing
 */
async function executeFixingStep(planPath, debugResult, options) {
  stateManager.updateState('FIXING', 'pending', 'Applying fix');
  
  if (options.verbose) {
    console.log(chalk.gray(`\n🔧 Fixing Step`));
  }
  
  try {
    // Check if fix plan exists
    if (!fs.existsSync(debugResult.fixPlanPath)) {
      const error = new Error('Fix plan not found');
      error.code = 'FIX_PLAN_NOT_FOUND';
      error.expectedPath = debugResult.fixPlanPath;
      throw error;
    }
    
    // Prompt user to apply fix (unless auto-fix)
    if (!options.autoFix) {
      const readline = require('readline');
      const rl = readline.createInterface({
        input: process.stdin,
        output: process.stdout
      });
      
      const answer = await new Promise(resolve => {
        rl.question(chalk.yellow('Apply fix? (Y/n): '), answer => {
          rl.close();
          resolve(answer);
        });
      });
      
      if (answer.toLowerCase() === 'n') {
        const error = new Error('Fix declined by user');
        error.code = 'FIX_DECLINED';
        throw error;
      }
    }
    
    // Execute fix plan using reis_executor subagent
    const attempt = stateManager.loadState()?.attempts || 1;
    const lastPlanPath = planPath;
    
    const fixResult = await invokeSubagent('reis_executor', {
      planPath: debugResult.artifacts?.fixPlanPath || '.planning/debug/FIX_PLAN.md',
      verbose: options.verbose,
      timeout: options.timeout || 600000,
      additionalContext: {
        isFix: true,
        originalPlan: lastPlanPath,
        attempt: attempt
      }
    });

    if (!fixResult.success) {
      throw new Error('Fix execution failed: ' + (fixResult.error?.message || 'Unknown error'));
    }
    
    stateManager.updateState('FIXING', 'success', 'Fix applied');
    
    if (options.verbose) {
      console.log(chalk.green('  ✓ Fix applied\n'));
    }
    
  } catch (error) {
    stateManager.updateState('FIXING', 'failure', error.message);
    
    // If user declined, re-throw as is
    if (error.code === 'FIX_DECLINED') {
      throw error;
    }
    
    // Otherwise wrap in fixing error
    const fixError = new Error(`Fix failed: ${error.message}`);
    fixError.code = 'FIX_FAILED';
    fixError.originalError = error;
    throw fixError;
  }
}

/**
 * Execute gate step - runs quality gates after verification
 * @param {string} planPath - Path to the plan file
 * @param {Object} options - Execution options
 * @returns {Object} Gate execution result
 */
async function executeGateStep(planPath, options = {}) {
  const { runAllGates, runCategoryGates } = require('../commands/gate');
  const { loadConfig } = require('./config');
  
  stateManager.updateState('GATING', 'pending', 'Running quality gates');
  
  if (options.verbose) {
    console.log(chalk.cyan('\n🛡️  Running Quality Gates...'));
  }
  
  try {
    // Check if gates should be skipped
    if (options.skipGates) {
      stateManager.updateState('GATING', 'skipped', 'Gates skipped by user');
      if (options.verbose) {
        console.log(chalk.yellow('  Gates skipped (--skip-gates flag)'));
      }
      return { success: true, skipped: true };
    }
    
    // Run gates using existing gate command infrastructure
    const gateOptions = {
      verbose: options.verbose,
      format: 'ascii'
    };
    
    let gateResult;
    if (options.gateOnly) {
      // Run specific category only
      gateResult = await runCategoryGates(options.gateOnly, gateOptions);
    } else {
      // Run all gates
      gateResult = await runAllGates(gateOptions);
    }
    
    // Store gate results in state
    stateManager.setGateResult(gateResult);
    
    if (gateResult.passed) {
      const msg = gateResult.hasWarnings ? 'Gates passed with warnings' : 'All gates passed';
      stateManager.updateState('GATING', 'success', msg);
      if (options.verbose) {
        console.log(chalk.green(`  ✅ ${msg}`));
      }
      return { success: true, ...gateResult };
    } else {
      const failCount = gateResult.summary?.failed || 0;
      stateManager.updateState('GATING', 'failure', `${failCount} gate(s) failed`);
      if (options.verbose) {
        console.log(chalk.red(`  ❌ ${failCount} gate(s) failed`));
      }
      return { success: false, ...gateResult };
    }
  } catch (error) {
    stateManager.updateState('GATING', 'failure', error.message);
    console.error(chalk.red(`  Gate error: ${error.message}`));
    return { success: false, error: error.message };
  }
}

/**
 * Format gate results as issues for debugging
 * @param {Object} gateResult - Gate execution result
 * @returns {string[]} Array of issue strings
 */
function formatGateIssues(gateResult) {
  const issues = [];
  if (gateResult.results) {
    for (const result of gateResult.results) {
      if (result.status === 'failed' || result.status === 'error') {
        issues.push(`[GATE:${result.category || result.gateName}] ${result.message}`);
        if (result.details && Array.isArray(result.details)) {
          for (const detail of result.details.slice(0, 3)) {
            const detailMsg = typeof detail === 'object' 
              ? (detail.message || detail.name || JSON.stringify(detail))
              : String(detail);
            issues.push(`  - ${detailMsg}`);
          }
        }
      }
    }
  }
  return issues;
}

/**
 * Step 4: Completion
 */
async function executeCompletionStep(planPath, verifyResult, options) {
  stateManager.updateState('COMPLETE', 'success', 'Cycle complete');
  
  // Update STATE.md
  await updateStateFile(planPath, options);
  
  // Clear cycle state
  stateManager.clearState();
  
  if (options.verbose) {
    console.log(chalk.green('\n✅ Cycle complete!\n'));
  }
}

/**
 * Update STATE.md with cycle completion
 */
async function updateStateFile(planPath, options) {
  const statePath = path.join(process.cwd(), '.planning', 'STATE.md');
  
  if (!fs.existsSync(statePath)) {
    console.warn(chalk.yellow('Warning: STATE.md not found, skipping update'));
    return;
  }
  
  // Get cycle state for completion record
  const cycleState = stateManager.loadState();
  const phase = cycleState?.phase || 'Unknown';
  
  // Calculate duration
  let duration = 'N/A';
  if (cycleState?.startTime) {
    const startTime = new Date(cycleState.startTime);
    const endTime = new Date();
    const durationMs = endTime - startTime;
    const minutes = Math.floor(durationMs / 60000);
    const seconds = Math.floor((durationMs % 60000) / 1000);
    duration = `${minutes}m ${seconds}s`;
  }
  
  // Build artifacts list
  const artifacts = [];
  if (cycleState?.executionResult?.artifacts) {
    artifacts.push(...cycleState.executionResult.artifacts);
  }
  if (planPath) {
    artifacts.push(planPath);
  }
  
  // Append completion record
  appendCompletionRecord(phase, {
    success: true,
    duration,
    attempts: cycleState?.attempts || 1,
    artifacts,
    completeness: cycleState?.completeness || 100
  });
}

/**
 * Append completion record to STATE.md
 * @param {string|number} phase - Phase identifier
 * @param {Object} cycleResult - Cycle result data
 */
function appendCompletionRecord(phase, cycleResult) {
  const statePath = path.join(process.cwd(), '.planning', 'STATE.md');
  
  const record = `
## Cycle Completion Record - Phase ${phase}

- **Completed:** ${new Date().toISOString()}
- **Duration:** ${cycleResult.duration || 'N/A'}
- **Status:** ${cycleResult.success ? '✓ Success' : '✗ Failed'}
- **Attempts:** ${cycleResult.attempts || 1}
- **Completeness:** ${cycleResult.completeness || 0}%
- **Artifacts:** ${(cycleResult.artifacts || []).join(', ') || 'None'}

`;

  if (fs.existsSync(statePath)) {
    fs.appendFileSync(statePath, record);
  }
}

/**
 * Handle graceful shutdown (Ctrl+C)
 */
function setupInterruptHandler() {
  process.on('SIGINT', () => {
    console.log(chalk.yellow('\n\n⚠️  Cycle interrupted by user'));
    
    const state = stateManager.loadState();
    if (state) {
      console.log(chalk.blue('State saved. Resume with: ') + chalk.cyan('reis cycle --resume\n'));
    }
    
    process.exit(130);
  });
}

// Setup interrupt handler
setupInterruptHandler();

/**
 * Show decision trees at phase transitions
 * @param {string} planPath - Path to plan file
 * @param {string} transition - Transition name (e.g., 'PLAN → EXECUTE')
 * @param {Object} options - Cycle options
 */
async function showDecisionTreesAtTransition(planPath, transition, options) {
  try {
    // Read plan content
    if (!fs.existsSync(planPath)) {
      return;
    }
    
    const planContent = fs.readFileSync(planPath, 'utf8');
    const trees = parseDecisionTrees(planContent);
    
    if (trees.length === 0) {
      return; // No trees to show
    }
    
    // Display header
    console.log('\n' + chalk.blue('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
    console.log(chalk.bold.cyan(`🔄 Phase Transition: ${transition}`));
    console.log(chalk.blue('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
    
    // Display each tree
    trees.forEach(tree => {
      console.log(renderDecisionTree(tree, {
        showMetadata: true,
        evaluateConditions: false
      }));
    });
    
    // Prompt to continue
    console.log(chalk.yellow('Continue with ' + transition.split(' → ')[1].toLowerCase() + '? (Y/n): '));
    
    // Wait for user input
    const readline = require('readline');
    const rl = readline.createInterface({
      input: process.stdin,
      output: process.stdout
    });
    
    const answer = await new Promise(resolve => {
      rl.question('', answer => {
        rl.close();
        resolve(answer);
      });
    });
    
    if (answer.toLowerCase() === 'n') {
      console.log(chalk.yellow('\nPhase transition cancelled by user.'));
      process.exit(0);
    }
    
    console.log(''); // Empty line for spacing
    
  } catch (error) {
    // Silently ignore errors - trees are optional
    if (options.verbose) {
      console.log(chalk.gray(`  Note: Could not parse decision trees: ${error.message}`));
    }
  }
}

module.exports = {
  runCycle,
  resumeCycle,
  executeReviewStep
};
